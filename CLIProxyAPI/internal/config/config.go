...
type Config struct {
	...
	// UsageStoreEnabled controls whether usage records are also written to the durable
	// usage store (usage.jsonl) for the Management API /usage endpoints. It only takes
	// effect when usage-statistics-enabled is also true, so existing deployments see no
	// change until usage statistics are turned on. Default: true.
	UsageStoreEnabled bool `yaml:"usage-store-enabled" json:"usage-store-enabled"`
	// UsageStoreMaxSizeMB caps the on-disk usage store size in megabytes. When the file
	// exceeds this size, the oldest records are pruned. 0 disables pruning (unbounded).
	// Default: 100.
	UsageStoreMaxSizeMB int `yaml:"usage-store-max-size-mb" json:"usage-store-max-size-mb"`
	// UsageStorePath overrides the usage store file location. When empty, the store is
	// written as usage.jsonl under the auth-dir. Relative paths are resolved against
	// auth-dir when non-empty.
	UsageStorePath string `yaml:"usage-store-path" json:"-"`
...
type RemoteManagement struct {
	...
	// DisableAutoUpdatePanel disables automatic periodic background updates and first-access
	// downloads of the management panel asset from GitHub. Defaults to true: the bundled local
	// panel (built from the repo's webui/ui and baked into the static directory) is served
	// without any outbound network fetch. Set to false to opt in to fetching an external panel
	// from PanelGitHubRepository.
	DisableAutoUpdatePanel bool `yaml:"disable-auto-update-panel"`
	// PanelGitHubRepository overrides the GitHub repository used to fetch an external management
	// panel asset. Only consulted when DisableAutoUpdatePanel is false. Accepts either a
	// repository URL (https://github.com/org/repo) or an API releases endpoint.
