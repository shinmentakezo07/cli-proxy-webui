...
func BuildConfigChangeDetails(oldCfg, newCfg *config.Config) []string {
	...
	if oldCfg.UsageStoreEnabled != newCfg.UsageStoreEnabled {
		changes = append(changes, fmt.Sprintf("usage-store-enabled: %t -> %t", oldCfg.UsageStoreEnabled, newCfg.UsageStoreEnabled))
	}
	if oldCfg.UsageStoreMaxSizeMB != newCfg.UsageStoreMaxSizeMB {
		changes = append(changes, fmt.Sprintf("usage-store-max-size-mb: %d -> %d", oldCfg.UsageStoreMaxSizeMB, newCfg.UsageStoreMaxSizeMB))
	}
	if strings.TrimSpace(oldCfg.UsageStorePath) != strings.TrimSpace(newCfg.UsageStorePath) {
		changes = append(changes, fmt.Sprintf("usage-store-path: %s -> %s", strings.TrimSpace(oldCfg.UsageStorePath), strings.TrimSpace(newCfg.UsageStorePath)))
	}
