...
func TestManagementUsageRequiresManagementAuthAndPopsArray(t *testing.T) {
	...
	// The durable /v0/management/usage endpoint is now implemented (returns the
	// persisted usage aggregate consumed by the web UI). With usage statistics
	// disabled the aggregate is empty but the route must still respond 200.
	aggregateReq := httptest.NewRequest(http.MethodGet, "/v0/management/usage", nil)
	aggregateReq.Header.Set("Authorization", "Bearer test-management-key")
	aggregateRR := httptest.NewRecorder()
	server.engine.ServeHTTP(aggregateRR, aggregateReq)
	if aggregateRR.Code != http.StatusOK {
		t.Fatalf("durable usage status = %d, want %d body=%s", aggregateRR.Code, http.StatusOK, aggregateRR.Body.String())
	}
	var aggregate struct {
		TotalRequests int64          `json:"total_requests"`
		Apis          map[string]any `json:"apis"`
	}
	if errUnmarshal := json.Unmarshal(aggregateRR.Body.Bytes(), &aggregate); errUnmarshal != nil {
		t.Fatalf("unmarshal durable usage response: %v body=%s", errUnmarshal, aggregateRR.Body.String())
	}
