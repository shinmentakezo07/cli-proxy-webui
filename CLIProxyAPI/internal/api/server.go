...
// managementPanelMissingMessage is shown when the bundled local panel asset is absent and
// external panel fetching is disabled. It points operators at the build script that bakes
// the local webui UI into the static directory.
const managementPanelMissingMessage = `<!doctype html><meta charset="utf-8"><title>Management panel not installed</title>
<h1>Management panel not installed</h1>
<p>The local management panel asset (management.html) was not found in the static directory.</p>
<p>Build and install it with <code>./scripts/build-panel.sh</code> (or the Docker image bakes it in automatically), then reopen this page.</p>
<p>To fetch an external panel from GitHub instead, set <code>disable-auto-update-panel: false</code> with a <code>panel-github-repository</code> in your config.</p>
`
...
func NewServer(cfg *config.Config, authManager *auth.Manager, accessManager *sdkaccess.Manager, configFilePath string, opts ...ServerOption) *Server {
	...
	applyUsageStoreConfig(cfg, hasManagementSecret || (cfg != nil && cfg.Home.Enabled))
...
func (s *Server) registerManagementRoutes() {
	...
	mgmt.Use(s.managementAvailabilityMiddleware(), s.mgmt.Middleware())
	{
		...
		mgmt.DELETE("/usage", s.mgmt.DeleteUsage)
...
func (s *Server) serveManagementControlPanel(c *gin.Context) {
	...
	if _, err := os.Stat(filePath); err != nil {
		if os.IsNotExist(err) {
			// When external panel fetching is disabled (the default), serve the bundled local
			// panel only. Do not perform an outbound GitHub fetch here; surface a helpful
			// message pointing at the build script instead of a bare 404.
			if cfg.RemoteManagement.DisableAutoUpdatePanel {
				c.Header("Content-Type", "text/html; charset=utf-8")
				c.String(http.StatusNotFound, managementPanelMissingMessage)
				c.Abort()
				return
			}
			// External panel opted in: synchronously ensure management.html is available with a
			// detached context. Control panel bootstrap should not be canceled by client disconnects.
...
func (s *Server) UpdateClients(cfg *config.Config) {
	...
	if oldCfg == nil || oldCfg.UsageStatisticsEnabled != cfg.UsageStatisticsEnabled ||
		oldCfg.UsageStoreEnabled != cfg.UsageStoreEnabled ||
		oldCfg.UsageStoreMaxSizeMB != cfg.UsageStoreMaxSizeMB ||
		oldCfg.UsageStorePath != cfg.UsageStorePath ||
		oldCfg.AuthDir != cfg.AuthDir {
		applyUsageStoreConfig(cfg, s.managementRoutesEnabled.Load() || (cfg != nil && cfg.Home.Enabled))
	}
...
// applyUsageStoreConfig configures the durable usage store from the loaded config.
// enabled mirrors the redisqueue enablement so the store only persists when management
// access (or home mode) is active. capture is additionally gated by usage-statistics-enabled.
func applyUsageStoreConfig(cfg *config.Config, enabled bool) {
	if cfg == nil {
		usagestore.DefaultStore().SetEnabled(false)
		return
	}
	usagestore.DefaultStore().SetConfig(
		cfg.UsageStorePath,
		cfg.AuthDir,
		cfg.UsageStoreMaxSizeMB,
		cfg.UsageStatisticsEnabled,
		enabled && cfg.UsageStoreEnabled,
	)
}
