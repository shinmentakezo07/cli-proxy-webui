...
### Management panel (local UI by default)

The web management panel served at `GET /management.html` is the **local React UI bundled with this repo** (`webui/ui`). The Docker image bakes it in automatically; when building the binary yourself, run:

```bash
./scripts/build-panel.sh
```

to build `webui/ui` and place `dist/index.html` at `static/management.html` so the server serves it.

By default the server does **not** fetch any panel from GitHub. To serve an external panel instead, set in `config.yaml`:
